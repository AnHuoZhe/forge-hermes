...
### 中审——模块级（product-reviewer 执行）

适用：当前模块所有任务完成后。

标准：
1. 模块内协作：所有零件拼一起不炸
2. I-O 正确：给定已知测试输入，模块整体输出符合预期
3. 不越层：依赖只在本层和下层
4. 测试覆盖：核心逻辑有测试锁住

不重复小审已覆盖的项（代码级接口签名和安全性）。

### 大审——系统级（product-reviewer 执行）

适用：全部模块完成。

标准：功能验证、架构一致性、契约检查、测试覆盖（核心路径+三个边界 case）、按方案裁减的六维度。

### 架构审查——设计级（product-reviewer 执行）

适用：阶段 B 产出架构文档后。采用五层裂隙分析（见审查维度），按方案规模裁减。六维度归入大审阶段。
...
### 六维度分析（仅涉及 Agent/LLM 设计时启用）

| 维度 | 检查点 |
|------|-------|
| 上下文工程 | 上下文静态结构设计、KV Cache/Chat Template 友好、Agent Skills 渐进式披露、Agent 状态栏、上下文压缩、动态上下文编排、提示注入防御（7 类） |
| 记忆系统 | 状态持久化和跨会话记忆 |
| 工具调用 | 工具定义能否让 LLM 正确选择 |
| 可靠性 | 输出校验和重试机制 |
| 成本控制 | token 消耗可控性 |
| 评估 | 系统是否正常工作的判断标准 |
...
每条发现必须同时包含"问题描述""为什么""修复方向"。没有对应项时写"无"，不省略章节。阻塞正确性、核心功能、依赖闭环、关键失败处理或选定架构要求的问题归入"必须改"；不阻塞交付但值得改进的归入"建议改"；其余归入"可忽略"。
